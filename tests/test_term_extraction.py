...
@pytest.mark.unit
def test_extract_term_definitions_accepts_english_iso_numbered_terms() -> None:
    text = """
...
NOTE A tester normally makes use of other functions such as data base management.
...
"""
    ...
    assert "function that is part of the tester" in terms["client"]
    assert "diagnostic data" in terms
    assert "data that is located in the memory" in terms["diagnostic data"]
