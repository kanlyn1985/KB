...
def test_closed_loop_tables_are_initialized(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert _columns(connection, "parse_views") >= {
            "view_id",
            "doc_id",
            "page_no",
            "view_type",
        ...
        assert _columns(connection, "page_parse_selection") >= {
            "doc_id",
            "page_no",
            "selected_view_id",
            "selected_reason",
...
def test_eval_run_summary_reads_evidence_shape_match_from_contract_metrics(tmp_path: Path) -> None:
    paths = initialize_workspace(tmp_path / "kb", SCHEMA_PATH)
    cases = [
        {
            "case_id": "CASE-SHAPE-1",
    ...
    try:
        ...
        eval_run_id = record_eval_run(
            connection,
            suite_id="regression:corpus_retrieval",
            cases=cases,
            summary={"total": 1, "passed": 1, "failed": 0},
            command="run-corpus-retrieval-eval",
            success=True,
            output="",
            case_results=[
                {
                    ...
                    "metrics": {
                        ...
                        "expected_evidence_shape": "process_activity",
                        "contract": {"evidence_shape_match": True},
                    },
                }
            ],
        )
        ...
        assert detail is not None
        summary = json.loads(detail["result_summary_json"])
        assert summary["evidence_shape_quality"]["total_expected"] == 1
        assert summary["evidence_shape_quality"]["shape_match_rate"] == 1.0
        assert summary["evidence_shape_quality"]["mismatch_count"] == 0
    finally:
        connection.close()
