...
def test_rewrite_treats_contextual_cp_meaning_as_definition() -> None:
    ...
    assert rewritten.query_type == "definition"
    assert rewritten.normalized_query != "CP"
    assert "控制导引功能" in rewritten.normalized_query
    assert "控制导引功能" in rewritten.target_topic
    assert "CP" in rewritten.protected_anchor_terms

    compact_rewritten = rewrite_query("CP控制导引是什么意思")
    assert compact_rewritten.query_type == "definition"
    assert compact_rewritten.normalized_query != "CP"
    assert "控制导引" in compact_rewritten.normalized_query
    assert "CP" in compact_rewritten.protected_anchor_terms
...
def test_answer_query_defines_contextual_cp_acronym(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert answer["context"]["rewrite"]["target_topic"] != "CP"
    assert answer["context"]["evidence_judgement"]["evidence_shape"] == "term_definition"
    assert answer["context"]["evidence_judgement"]["shape_diagnostics"]["shape_contract"]["allowed_shapes"] == ["term_definition"]
