...
- **默认套件状态（Sprint 3 P3 后，2026-06-26）**：`705 passed, 1 skipped, 0 failed, 0 xfailed`（~5min，默认 deselect integration+benchmark）。全量套件（`-o addopts=""` 取消 deselect）含大量 integration/benchmark，会很慢且有失败，正常回归不要跑全量。
...
- **eval 主口径**：`eakb eval run-now --suite golden --version v1 --max-questions 20`（token_overlap，无 LLM，跨文档轮询采样）。Sprint 3 P3 后当前诚实基线 **20 题真实 pass_rate=0.40（0 artifact）**；CI smoke floor `--min-token-pass 0.20`。`EVAL_USE_LLM=1` 才启用 LLM judge（辅助非阻塞）。10 题样本太小（恒 0.30），已扩到 20 题以稳定反映质量。详见 `docs/dev/eval-baseline-policy.md`。
