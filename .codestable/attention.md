...
- **默认套件状态（Sprint 3 P3+[6] 后，2026-06-26）**：`711 passed, 1 skipped, 0 failed, 0 xfailed`（~5min，默认 deselect integration+benchmark）。全量套件（`-o addopts=""` 取消 deselect）含大量 integration/benchmark，会很慢且有失败，正常回归不要跑全量。
...
- **Sprint 3（2026-06-26 进行中）**：WP0 baseline 冻结 + WP1 failure taxonomy（4 次根因修正）完成；WP2 P0 硬降级（`_should_downgrade_for_insufficient_evidence` sufficient=False AND conf<0.2 时降级）+ doc-selection fix（`_choose_doc_from_evidence_judgement` best_fact_ids=[] 时回退 best_evidence_ids 投票）+ P3 噪声题收紧 + metric 防降级完成。诚实真实基线 20 题 pass_rate=0.35（[6] 元数据降权后）。WP3 query_rewrite 锚点拆分（整句 must_term→should_terms 锚点）让 [4][12] 正确文档进入候选（召回改善），pass_rate 无变化无回归。
- **eval 主口径**：`eakb eval run-now --suite golden --version v1 --max-questions 20`（token_overlap，无 LLM，跨文档轮询采样）。Sprint 3 P3 后当前诚实基线 **20 题真实 pass_rate=0.35（0 artifact）**；CI smoke floor `--min-token-pass 0.20`。`EVAL_USE_LLM=1` 才启用 LLM judge（辅助非阻塞）。10 题样本太小（恒 0.30），已扩到 20 题以稳定反映质量。详见 `docs/dev/eval-baseline-policy.md`。
