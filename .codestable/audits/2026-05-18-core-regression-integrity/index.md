...
| ID | Status | Note |
|---|---|---|
| F-04 | partial | 当前 7 个 user-style golden 的 query_type 分配已与代码分类一致；但审计建议的"行为合同"升级（从 expected_query_type 硬约束改为行为级校验）尚未实施。已纳入 kb1-next-phase roadmap。 |
| F-05 | mitigated | test_delivery_assets.py 和 demo.html 当前 tab 集一致；但测试仍断言具体字符串而非页面结构。已纳入 kb1-next-phase roadmap 作为 P2 项。 |
