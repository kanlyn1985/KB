...
本 roadmap 覆盖六个闭环的工程化强化：

- 入库闭环：source_units、coverage report、ingestion acceptance。
- 解析质量闭环：parse views、parse risk profiling、document diagnostics、parse risk actions。
...
- 派生状态治理闭环：derived state registry、freshness guard、workspace doctor、rebuild、run governance。

解析质量闭环和派生状态治理闭环的详细规划分别在各自的 roadmap 中定义。本 roadmap 聚焦入库、召回、答案、回归四个闭环的工程化，以及各闭环间的接口契约。
...
| done | golden-generation-v1 | 统一 golden 自动生成、置信分级、准入门、报告和审核闭环。 |
...
- 2026-05-12：完成 golden-generation-v1，承接 corpus eval、coverage promotion 和 failure draft 的统一生成契约，并提供 CLI/API/Workbench dry-run review payload。
- 2026-05-19：roadmap 从"四闭环"升级为"六闭环"命名，补齐解析质量闭环和派生状态治理闭环到范围说明。
