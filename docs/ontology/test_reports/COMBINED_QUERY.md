# Phase 6 (Bridge) Test Report: Combined Legacy + Ontology
...
[Router]  (ontology/combined_query.py)
   ...
   ↓
┌─ Ontology handler ──────────┐   ┌─ Legacy handler ────────┐
| structured queries         |   | answer_api.answer_query |
| (parameter/ref/traversal) |   | (LLM + FTS pipeline)   |
└────────────────────────────┘   └─────────────────────────┘
   ↓                                  ↓
   ontology_answer                    legacy_excerpt
   ↓                                  ↓
...
| ``src/kb1_ontology/combined_query.py`` | Router + handlers + combiner. Routes to the right system, runs both, merges results. |
| ``src/kb1_ontology/tests/test_combined_query.py`` | 14 tests covering router, structure, real data, error handling. |
| ``scripts/ontology_demo/combined_query_demo.py`` | Runnable demo showing 10 questions through the combined system. |
...
|----------|---------|---------|
| `parameter` | "P2 timing", "rated voltage", "50 ms" | Ontology (typed) |
| `reference` | "references", "引用了", "depends on" | Ontology (set) |
| `traversal` | "2 跳可达", "2-hop" | Ontology (BFS) |
| `service` | "UDS 服务", "0x10" | Ontology (list) |
| `definition` | "是什么", "定义是什么" | Ontology (title) |
...
```
Test counts:    14 / 14 passed
Total kb1_ontology tests: 143 / 143 passed
KB1 main system:  unaffected
```

Tests cover:
- Router classification for all 6 categories
- CombinedAnswer dataclass structure
- Real-data queries for each category
- Error handling (empty, nonsense, unicode)
- The bridge being a documented exception to T2
...
- The legacy pipeline is slow (3-30s per query) because it goes
  ...
  use ontology first and only fall back to legacy for
  free-form questions.
