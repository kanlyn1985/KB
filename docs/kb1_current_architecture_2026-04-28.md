...
这四个闭环是产品层定义。后续模块、数据库表、API、UI 和测试都应服务于这四个闭环。
...
这些表应作为 additive schema 演进，不要求一次性全部实现，但设计上必须承认它们是四个闭环的基础设施。
