...
def build_graph_for_document(workspace_root: Path, doc_id: str) -> GraphBuildResult:
    ...
    try:
        ...
        _delete_doc_graph_artifacts(connection, doc_id)
...
def _delete_doc_graph_artifacts(connection, doc_id: str) -> dict[str, int]:
    edge_rows = connection.execute(
        "SELECT edge_id FROM graph_edges WHERE version_scope = ?",
        (doc_id,),
    ).fetchall()
    edge_ids = [str(row["edge_id"]) for row in edge_rows]
    if not edge_ids:
        return {"edge_evidence_map": 0, "graph_edges": 0}
    placeholders = ",".join("?" for _ in edge_ids)
    evidence_cursor = connection.execute(
        f"DELETE FROM edge_evidence_map WHERE edge_id IN ({placeholders})",
        edge_ids,
    )
    ...
    return {
        "edge_evidence_map": max(int(evidence_cursor.rowcount or 0), 0),
        "graph_edges": max(int(edge_cursor.rowcount or 0), 0),
    }
