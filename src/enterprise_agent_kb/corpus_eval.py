...
SUPPORTED_CASE_TYPES = ("definition", "parameter", "process_activity", "requirement")
ACTIONABLE_SOURCE_UNIT_QUALITY_REASONS = ("weak_definition_shape",)
...
def analyze_source_unit_candidate_quality(
    db_file: Path,
    *,
    doc_ids: list[str] | None = None,
    case_types: list[str] | None = None,
    ...
    selected_types = _selected_case_types(case_types)
    rows = _load_source_unit_rows(db_file, doc_ids=doc_ids)
    reason_counts: dict[str, int] = {}
    samples: dict[str, list[dict[str, object]]] = {}
    for row in rows:
        reason = _skip_reason(row, selected_types)
        if not reason or reason not in ACTIONABLE_SOURCE_UNIT_QUALITY_REASONS:
            continue
...
def run_corpus_retrieval_eval(
    ...
    *,
    ...
    for index, case in enumerate(cases, start=offset + 1):
        started = time.perf_counter()
        result = _evaluate_case(paths.root, case, index=index, limit=limit)
        ...
        if progress:
            _emit_progress_event(index=index, total=len(all_cases), result=result, duration_seconds=duration_seconds)
    ...
    summary = _eval_summary(
        ...
        case_results,
        cases_path,
        generated.summary if generated else None,
    ...
    try:
        sync_golden_cases(connection, CORPUS_EVAL_SCOPE_ID, all_cases, source="corpus_eval")
        eval_run_id = record_eval_run(
            connection,
            suite_id=suite_id,
            cases=cases,
            summary=summary,
            command=_eval_command(cases_path, limit, case_limit, case_offset, generation_limit_per_type),
            success=failed == 0,
...
def _case_from_source_unit(row: Row, *, selected_types: set[str]) -> dict[str, object] | None:
    ...
    if row_kind == "requirement":
        return _requirement_case(row)
    return None
...
def _skip_reason(row: Row, selected_types: set[str]) -> str:
    ...
    if row_kind == "definition" and not _has_definition_evidence_shape(row):
        return "weak_definition_shape"
...
def _row_case_type(row: Row) -> str:
    structural_blob = _row_structural_blob(row)
    full_blob = _row_descriptor_blob(row)
    if "definition" in structural_blob or "term_definition" in structural_blob:
        return "definition"
    if "parameter_row" in structural_blob or "parameter" in structural_blob and "|" in str(row["text"] or ""):
        return "parameter"
    if "process_practice" in structural_blob or ("process" in full_blob and re.search(r"\b[A-Z]{2,6}\.\d+\.BP\d+\b", str(row["text"] or ""), re.I)):
        return "process_activity"
    if "requirement" in structural_blob:
        return "requirement"
    return ""
...
def _row_structural_blob(row: Row) -> str:
    return " ".join(
        str(row[name] or "").casefold()
        for name in ("unit_type", "content_role", "expected_knowledge_type")
    )
...
def _has_definition_evidence_shape(row: Row) -> bool:
    ...
    if not text:
        return False
    if _looks_like_name_list_anchor(anchor):
        return False
    if _looks_like_figure_legend_definition(anchor, text):
        return False
    cue_pattern = r"[:：]|是指|是指由|指的是|指|表示|定义为|定义成|称为|为|\bis\s+(?:a|an|the)\b|\bare\s+(?:a|an|the)\b|means|refers to|defined as"
    prefix = text[: min(len(text), 160)]
    if re.search(cue_pattern, prefix, flags=re.I):
        return True
    if _looks_like_term_anchor(anchor) and _definition_has_publishable_signal(text):
        return True
    if _looks_like_term_anchor(anchor) and _looks_like_definition_body(text):
        return True
    return False


def _looks_like_term_anchor(anchor: str) -> bool:
    text = _clean_text(anchor)
    if not text:
        return False
    has_cjk = bool(re.search(r"[\u4e00-\u9fff]", text))
    has_latin = bool(re.search(r"\b[A-Za-z][A-Za-z0-9+/\-]{1,}\b", text))
    if has_cjk and has_latin:
        return True
    if re.fullmatch(r"[A-Za-z][A-Za-z0-9+/\-]*(?:\s+[A-Za-z][A-Za-z0-9+/\-]*){0,5}", text):
        return True
    if re.fullmatch(r"[A-Z][A-Z0-9+/\-]{1,12}", text):
        return True
    return False


def _looks_like_definition_body(text: str) -> bool:
    value = _clean_text(text)
    if len(value) < 8:
        return False
    if not re.search(r"[\u4e00-\u9fff]", value):
        return False
    if re.match(r"^(标引序号说明|参考文献|作者|摘要|关键词)\b", value):
        return False
    return True


def _looks_like_name_list_anchor(anchor: str) -> bool:
    text = _clean_text(anchor)
    if not re.search(r"[，,、]", text):
        return False
    parts = [_clean_text(part) for part in re.split(r"[，,、]", text) if _clean_text(part)]
    if len(parts) < 2 or len(parts) > 6:
        return False
    return all(2 <= len(part) <= 4 and re.fullmatch(r"[\u4e00-\u9fff]+", part) for part in parts)


def _looks_like_figure_legend_definition(anchor: str, text: str) -> bool:
    title = _clean_text(anchor)
    body = _clean_text(text)
    if re.search(r"(标引序号|——|☆)", title) or re.match(r"^(标引序号说明|图\s*\d+|表\s*\d+)", body):
        return True
    if len(re.findall(r"[\u4e00-\u9fff]{2,}", title)) >= 4 and " " in title and len(body) < 80:
        return True
    return False
...
def _case_anchor(row: Row, row_kind: str) -> str:
    ...
    if row_kind == "requirement":
        return _requirement_anchor(row)
    return ""
...
def _requirement_case(row: Row) -> dict[str, object] | None:
    anchor = _requirement_anchor(row)
    if not anchor:
        return None
    return _base_case(
        row,
        case_type="requirement",
        query=f"{_query_anchor(anchor)}有哪些要求？",
        expected_query_type="constraint",
        expected_evidence_shape="requirement",
        retrieval_must_hit=_dedupe_anchors([anchor, _requirement_detail_phrase(row)]),
    )
...
def _requirement_anchor(row: Row) -> str:
    title = _requirement_topic_anchor(_clean_text(row["canonical_key"]) or _clean_text(row["canonical_title"]))
    if _usable_anchor(title):
        return title
    text = _clean_text(row["text"])
    for sentence in re.split(r"[。；;\n]", text):
        sentence = _requirement_topic_anchor(sentence)
        if _usable_anchor(sentence):
            return sentence
    return ""


def _requirement_topic_anchor(value: str) -> str:
    text = _strip_section_prefix(_clean_text(value))
    text = re.split(r"\bTable\s+\d+\s+(?:specifies|shows|lists|contains)\b", text, maxsplit=1, flags=re.I)[0]
    text = re.split(r"\bFigure\s+\d+\s+(?:shows|illustrates|specifies)\b", text, maxsplit=1, flags=re.I)[0]
    text = re.split(r"\b(?:specifies|shows|lists)\s+the\b", text, maxsplit=1, flags=re.I)[0]
    return _truncate_anchor(text, max_chars=120)


def _requirement_detail_phrase(row: Row) -> str:
    text = _clean_text(row["text"])
    for phrase in re.split(r"[。；;\n]", text):
        phrase = _clean_text(phrase)
        if 6 <= len(phrase) <= 80:
    ...
    return {
        ...
        "metrics": {
            **trace_metrics,
            ...
            "evidence_shape_match": contract.get("evidence_shape_match"),
            "shape_contract_matched": trace_metrics.get("shape_contract_matched"),
...
def _eval_summary(
    ...
    *,
    ...
    for item in case_results:
        ...
        duration = _float_or_none(metrics.get("duration_seconds"))
    ...
    return {
        ...
        "evaluation_window": {
            "total_case_count": int(total_case_count if total_case_count is not None else len(cases)),
...
def _emit_progress_event(*, index: int, total: int, result: dict[str, object], duration_seconds: float) -> None:
    event = {
        "event": "corpus_case_evaluated",
        "case_index": index,
...
def _duration_summary(durations: list[float]) -> dict[str, object]:
    if not durations:
        return {
            "case_count": 0,
            "total_seconds": 0.0,
    ...
    return {
        ...
        "total_seconds": total,
        "average_seconds": round(total / len(durations), 3),
...
def _query_anchor(anchor: str) -> str:
    ...
    return _truncate_anchor(value, max_chars=120 if not re.search(r"[\u4e00-\u9fff]", value) else 48)


def _truncate_anchor(value: str, *, max_chars: int) -> str:
    text = _clean_text(value)
    if len(text) <= max_chars:
        return text
    truncated = text[:max_chars].rstrip()
    if re.search(r"[A-Za-z]$", truncated) and len(text) > max_chars and re.match(r"[A-Za-z]", text[max_chars : max_chars + 1]):
...
def _case_fingerprint(case: dict[str, object]) -> str:
    must_hit = _string_list(case.get("retrieval_must_hit") or case.get("must_hit"))
    primary_anchor = must_hit[0] if must_hit else ""
    parts = [
        str(case.get("expected_doc_id") or ""),
        str(case.get("case_type") or ""),
        _normalize_for_contains(case.get("query")),
        _normalize_for_contains(primary_anchor),
    ]
    return "|".join(parts)
