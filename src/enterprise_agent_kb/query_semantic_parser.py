...
def _ensure_provider_available(provider_name: str) -> None:
    failure_until = _PROVIDER_FAILURE_UNTIL.get(provider_name, 0.0)
...
def _mark_provider_failure(provider_name: str) -> None:
    cooldown = _provider_failure_cooldown_seconds()
    if cooldown:
        _PROVIDER_FAILURE_UNTIL[provider_name] = time.monotonic() + cooldown
...
def _call_minimax_text(prompt: str, system_prompt: str = SEMANTIC_PARSER_SYSTEM_PROMPT) -> str:
    _ensure_provider_available("minimax")
