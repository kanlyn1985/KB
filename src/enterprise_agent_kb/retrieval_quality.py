...
def evaluate_retrieval_quality(
    *,
    ...
    for original, anchor in zip(negative_expected, normalized_negative, strict=False):
        rank = _first_matching_rank(anchor, normalized_items, allow_token_fallback=False)
        if rank is not None:
    ...
    recall_by_k = {
        k: round(sum(1 for rank in matched_ranks if rank <= k) / max(1, must_total), 6)
        for k in k_values
    }
    min_recall_at_5 = _float_value(case.get("min_recall_at_5"), default=0.8)
    result: dict[str, object] = {
        ...
        "failure_attribution": _failure_attribution(
            ...
            best_rank=best_rank,
            negative_hits=negative_hits,
            recall_at_5=recall_by_k.get(5),
            min_recall_at_5=min_recall_at_5,
            trace_metrics=metrics,
        ),
    }
    for k, recall in recall_by_k.items():
...
def _failure_attribution(
    *,
    ...
    if best_rank and best_rank > 5:
        return "rank_too_low"
    if must_total and recall_at_5 is not None and recall_at_5 < min_recall_at_5:
        return "recall_at_5_below_threshold"
    return "ok"
...
def _first_matching_rank(anchor: str, normalized_items: list[str], *, allow_token_fallback: bool = True) -> int | None:
...
def _item_text(item: dict[str, object]) -> str:
    return _strip_internal_retrieval_labels(str(item.get("snippet") or ""))


def _strip_internal_retrieval_labels(text: str) -> str:
    value = str(text or "")
    value = re.sub(
        r"^(?:graph_fact|knowledge_unit_fact|process_sibling_fact|has_table_requirement)\s+(?:[a-z_]+:\s*)?",
        "",
        value.strip(),
        flags=re.I,
    )
