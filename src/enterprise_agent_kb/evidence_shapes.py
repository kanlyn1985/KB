...
SHAPE_CONTRACTS: dict[str, EvidenceShapeContract] = {
    ...
    "constraint": EvidenceShapeContract("constraint", ("parameter_definition", "process_activity", "requirement")),
...
def allowed_evidence_shapes_for_query(query: str, query_type: str) -> tuple[str, ...]:
    allowed = allowed_evidence_shapes(query_type)
    if str(query_type or "").strip() == "definition" and allowed:
        if is_parameter_definition_query(query):
            return allowed
        return tuple(shape for shape in allowed if shape == "term_definition")
    return allowed
...
def is_requirement_query(query: str) -> bool:
    text = str(query or "")
    if is_test_method_query(text) or is_parameter_definition_query(text) or is_process_activity_query(text):
        return False
    return bool(re.search(r"(要求|规定|应当|应|shall|requirement|requirements)", text, re.I))
...
def _looks_like_requirement_blob(query: str, blob: str) -> bool:
    if is_preface_or_index_blob(blob):
        return False
    normalized = normalize(blob)
    query_tokens = _query_anchor_tokens(query)
...
def _requirement_score(query: str, _kind: str, item: dict[str, object], blob: str) -> float:
    if not _looks_like_requirement_blob(query, blob):
        return 0.0
    fact_type = str(item.get("fact_type") or "")
    if fact_type in {"requirement", "table_requirement"}:
        return 1.25
    if "requirement" in str(item.get("graph_relation") or ""):
        return 1.0
    return 0.65


def _requirement_sufficient(query: str, scored: list[SCORED_ROW]) -> bool:
    return any("requirement" in row[4] and _looks_like_requirement_blob(query, _blob(row[2])) for row in scored[:5])
...
def _object_anchor_terms(query: str) -> tuple[list[str], list[str]]:
    if re.search(r"(?<![A-Za-z0-9])OBC(?![A-Za-z0-9])|车载充电机|on-?board charger", str(query or ""), re.I):
        return (
            ["OBC", "车载充电机", "电动汽车用传导式车载充电机", "on-board charger", "onboard charger"],
            [] if "逆变" in str(query or "") else ["汽车电源逆变器", "逆变器", "逆变"],
        )
    return ([], [])
...
EVIDENCE_SHAPES: list[EvidenceShape] = [
    ...
    EvidenceShape(
        name="requirement",
        applies=is_requirement_query,
        score_candidate=_requirement_score,
        is_sufficient=_requirement_sufficient,
        reason="top evidence covers requirement facts or requirement table rows",
    ),
]
