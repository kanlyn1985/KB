"""Workspace health snapshots: hygiene loop, parse risk, coverage, graph, etc.

Extracted from `api_server._impl` to isolate the workspace-status
query/snapshot logic (per-table/per-section health aggregations
and per-loop risk scoring) from the HTTP request handler and the
route-dispatch/server lifecycle.
"""
...
from ..closed_loop_store import ensure_source_unit_mapping_tables, list_eval_runs
from ..closed_loop_store._runtime import _runtime_code_version
...
def _hygiene_loop_snapshot(workspace_root: Path) -> dict[str, Any]:
    ...
    except (RuntimeError, ValueError, IOError, OSError) as exc:
        loop: dict[str, Any] = {
            "doctor_status": "fail",
            "issue_count": 1,
