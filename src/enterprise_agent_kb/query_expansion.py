...
def _extract_hard_anchors(query: str) -> list[str]:
    ...
    def add(value: str) -> None:
        ...
        if re.fullmatch(r"[a-z]{1,6}\d*", cleaned):
            return
...
def _should_use_rule_expansion(query: str, anchors: list[str]) -> bool:
    ...
    if re.search(r"(有哪些活动|要做哪些活动|活动要做|有哪些任务|有哪些步骤|步骤有哪些|任务有哪些|过程域.*(?:是什么|定义|含义))", query, re.I):
        return True
    if re.search(r"(有哪些要求|有什么要求|要求有哪些|要求是什么|应满足什么|应符合什么|不应超过什么|不小于什么)", query, re.I):
        return True
...
def _fallback_expansion(query: str, anchors: list[str]) -> QueryExpansion:
    ...
    elif re.search(r"(有哪些要求|有什么要求|要求有哪些|要求是什么|应满足什么|应符合什么|不应超过什么|不小于什么)", query, re.I):
        topic = _strip_constraint_intent_suffix(query)
        intent_candidates = ["requirement_lookup", "constraint_lookup"]
        expanded_terms = [topic, "要求", "requirement", "requirements", "shall", *anchors]
        possible_answer_shape = "requirement_set"
...
def _strip_constraint_intent_suffix(query: str) -> str:
    ...
    text = re.sub(r"(?:的)?要求$", "", text, flags=re.I).strip()
    return text
