...
def rerank_candidates(
    ...
    try:
        ...
        if rewritten.query_type == "lifecycle_lookup":
            reranked = _diversify_lifecycle_candidates(rewritten, reranked)
...
def _diversify_lifecycle_candidates(rewritten: RewrittenQuery, candidates: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for candidate in candidates:
        ...
        preference = float(candidate.get("score") or 0.0) + _lifecycle_duplicate_preference(rewritten, snippet)
        current = best_by_code.get(bp_code)
        if current is None or preference > current[0]:
            best_by_code[bp_code] = (preference, candidate)
    selected: list[dict[str, object]] = []
    for code in ordered_codes:
        candidate = best_by_code[code][1]
        selected.append(candidate)
        selected_ids.add(str(candidate.get("result_id") or ""))
    selected.extend(candidate for candidate in candidates if str(candidate.get("result_id") or "") not in selected_ids)
    return selected
...
def _lifecycle_duplicate_preference(rewritten: RewrittenQuery, snippet: str) -> float:
    query_text = " ".join(
        str(item or "")
        for item in [
    ...
    for token in ("分析软件架构", "软件架构分析", "定义软件架构", "软件架构设计", "系统集成", "集成验证"):
        if token in query_norm and token in snippet_norm:
            preference += 0.35
    return preference
    ...
    if result_type == "fact" and result_id:
        ...
        if rewritten.query_type in {"lifecycle_lookup", "timing_lookup"}:
            if fact_type == "process_fact":
                ...
                bonus = 0.55
                if rewritten.query_type == "lifecycle_lookup":
                    bonus += _lifecycle_focus_bonus(rewritten, str(row["object_value"] or ""))
                return bonus
            if fact_type == "table_requirement":
                if _looks_like_bp_table(payload_blob):
                    return 0.62 + _lifecycle_focus_bonus(rewritten, str(row["object_value"] or ""))
...
def _parameter_subtype_bonus(rewritten: RewrittenQuery, fact_type: str, payload: str) -> float:
    ...
    intent_bonus = _parameter_intent_bonus(rewritten, payload)
...
def _parameter_intent_bonus(rewritten: RewrittenQuery, payload: str) -> float:
    query_text = " ".join(
        str(item or "")
        for item in [
            rewritten.original_query,
    ...
    if not re.search(r"(电阻|阻值|等效电阻|欧姆|Ω|ohm|resistance)", query_text, re.I):
        return 0.0
    if _parameter_payload_has_resistance(payload):
        return 0.72
    return -0.85


def _parameter_payload_has_resistance(payload: str) -> bool:
    text = str(payload or "")
    if re.search(r"(电阻|阻值|等效电阻|Ω|\\\\Omega|欧姆|ohm|resistance)", text, re.I):
        return True
    data = _safe_json_object(text)
    if not isinstance(data, dict):
        return False
    symbol = _clean_parameter_field(data.get("symbol"))
    unit = _clean_parameter_field(data.get("unit"))
    parameter = _clean_parameter_field(data.get("parameter"))
    return bool(unit in {"Ω", "\\Omega"} or re.match(r"R\d", symbol, re.I) or re.search(r"(电阻|阻值|resistance)", parameter, re.I))
...
def _query_contains_symbol(query_text: str, symbol: str) -> bool:
    ...
    if len(symbol) < 2:
        return False
    if re.fullmatch(r"[A-Za-z]{2,}", symbol):
        query_acronyms = set(re.findall(r"(?<![A-Za-z0-9])([A-Z]{2,})(?![A-Za-z0-9])", query_text))
        if query_acronyms:
            return symbol in query_acronyms
...
def _object_anchor_adjustment(connection, rewritten: RewrittenQuery, candidate: dict[str, object], snippet: str) -> tuple[float, float]:
    ...
    content_haystack = _norm(snippet)
    filename_haystack = _norm(filename)
    bonus = 0.0
    penalty = 0.0
    content_has_positive = positives and any(_norm(term) in content_haystack for term in positives)
    filename_has_positive = positives and any(_norm(term) in filename_haystack for term in positives)
    content_has_negative = negatives and any(_norm(term) in content_haystack for term in negatives)
    if content_has_positive:
        bonus += 1.0
    elif filename_has_positive:
        bonus += 0.2
    if content_has_negative and not content_has_positive:
        penalty += 1.35
...
def _object_anchor_terms(rewritten: RewrittenQuery) -> tuple[list[str], list[str]]:
    ...
    if re.search(r"(?<![A-Za-z0-9])OBC(?![A-Za-z0-9])|车载充电机|on-?board charger", text, re.I):
        return (
            ["OBC", "车载充电机", "电动汽车用传导式车载充电机", "on-board charger", "onboard charger"],
            [] if "逆变" in text else ["汽车电源逆变器", "逆变器", "逆变"],
        )
    return ([], [])
...
def _looks_like_bp_table(payload_blob: str) -> bool:
    return bool(re.search(r"\b(?:SYS|SWE|SUP|MAN|HWE|VAL|REU|PIM)\.\d+\.BP\d+\b", payload_blob, re.I))


def _lifecycle_focus_bonus(rewritten: RewrittenQuery, payload: str) -> float:
    query = str(rewritten.original_query or rewritten.normalized_query or rewritten.target_topic or "")
    blob = _norm(payload)
    bonus = 0.0
    focus_terms = (
        ("分析", ("分析", "analyze")),
        ("设计", ("设计", "specify", "architecturaldesign", "architecture")),
        ("定义", ("定义", "specify")),
        ("集成", ("集成", "integrat")),
        ("验证", ("验证", "verify", "verification")),
        ("测试", ("测试", "test")),
    )
    for query_term, payload_terms in focus_terms:
        if query_term in query and any(_norm(term) in blob for term in payload_terms):
            bonus += 0.28
    return min(bonus, 0.56)
