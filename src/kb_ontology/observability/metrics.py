"""In-process counters and duration summaries (no external deps).

Adapted from agent_kb_core observability/metrics.
"""
...
class MetricSnapshot:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "durations_ms": {
                name: dict(values) for name, values in self.durations_ms.items()
            },
        }


class MetricsRegistry:
    """Thread-safe counters and duration summaries."""
    ...
    def snapshot(self) -> MetricSnapshot:
        with self._lock:
            ...
            durations = {
                name: _summarize(values) for name, values in self._durations.items()
            }
