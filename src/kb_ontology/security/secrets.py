"""Secret providers and rotating API-key authenticator.

Adapted from agent_kb_core security/secrets (copy, not import).
"""
...
from kb_ontology.security.auth import APIKeyAuthenticator, Principal
...
class HTTPSecretProvider:
    ...
    def get_secret(self, name: str) -> str:
        ...
        try:
            ...
            raise RuntimeError(
                f"secret manager request failed: {type(exc).__name__}"
            ) from exc
...
class RotatingAPIKeyAuthenticator:
    ...
    def __init__(
        ...
        *,
        secret_name: str = "KB_ONTOLOGY_API_KEYS",
    ...
    def refresh(self, *, force: bool = False, now: float | None = None) -> bool:
        ...
        if (
            not force
            and timestamp < self._next_refresh
