"""API-key authentication, RBAC, and tenant database routing.

Adapted from agent_kb_core security/auth (copy, not import).
"""
...
ROLE_PERMISSIONS: dict[str, frozenset[str]] = {
    "reader": frozenset(
        {"health:read", "query:run", "metrics:read", "jobs:read"}
    ),
...
class APIKeyAuthenticator:
    ...
    Raw keys are hashed at construction and never returned. Environment
    variable ``KB_ONTOLOGY_API_KEYS`` (preferred) or ``AGENT_KB_API_KEYS``
    accepts a JSON object: token → {principal_id, tenant_id, roles}.
    """
    ...
    def from_environment(
        ...
        names = (
            [variable]
            if variable
            else ["KB_ONTOLOGY_API_KEYS", "AGENT_KB_API_KEYS"]
        )
        raw = ""
        used = ""
        for name in names:
        ...
        if not raw:
            raise ValueError(
                "KB_ONTOLOGY_API_KEYS (or AGENT_KB_API_KEYS) is not configured"
            )
        payload = json.loads(raw)
        if not isinstance(payload, dict):
            raise ValueError(f"{used} must be a JSON object")
...
class TenantDatabaseRouter:
    """Physical tenant isolation: one SQLite file per tenant."""
    ...
    def list_tenants(self) -> list[str]:
        return sorted(
            path.stem for path in self.root_dir.glob("*.sqlite3") if path.is_file()
        )


def normalize_tenant_id(value: str) -> str:
    normalized = (
        re.sub(r"[^a-zA-Z0-9_-]+", "-", str(value or "").strip()).strip("-").lower()
    )
